...
function scr_determine_direction_facing() {
	...
	if (!stunActive) && (!hitstunActive) && ((enemyState != enemystates.heavyMeleeAttack) && (enemyState != enemystates.lightMeleeAttack) && (enemyState != enemystates.heavyRangedAttack) && (enemyState != enemystates.lightRangedAttack) || (enemyState = enemystates.idle)) && (obj_skill_tree.solidifyTarget != self.id) && (enemyTotalSpeed > 0) {
		...
		//if chosenEngine != "" {
			...
			// a random direction if its target is itself.
				...
				if instance_exists(currentTargetToHeal) {
					if currentTargetToHeal != self.id {
